...
public class Client {
    ...
    public boolean Afegir_clients() throws SQLException, ParseException, IOException{
            ...
            System.out.println(datObj.calcularEdad(fecha));
            do{
                System.out.println("--------------------------------");
    ...
    public ArrayList<Client> llistarReserves(){
       try{
           Connection con = new Conexio_bd().connectarBD();
           String consulta = "select distinct cl.Nom, cl.Cognom, cl.DNI, cl.Correu, cl.Adreça, cl.Telefon, cl.CC, cl.Contrassenya, cl.Data, SUM(res.Num_Reserves) FROM client cl, reserves res WHERE cl.DNI=res.DNI ORDER by res.Num_Reserves ASC";
           PreparedStatement sql = con.prepareStatement(consulta);
           ResultSet rs = sql.executeQuery();
           ArrayList<Client> llista = new ArrayList();
           Client c = new Client();
    ...
    public void actDia() throws SQLException{
        ...
        LocalDate date = LocalDate.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        String consulta2 = String.format("select a.*, s.* from activitat a, sala s where a.ID_s=s.ID AND a.Data_fisica= " + "\"%s\"" + " ORDER BY Reserves DESC;", date.format(formatter));
        PreparedStatement sql2 = con.prepareStatement(consulta2);
        sql2.executeQuery();
        ResultSet rs2 = sql2.executeQuery();
        while (rs2.next()) {
            ID_a = rs2.getInt("ID_Activitat");
            descripcio = rs2.getString("Descripció");
            int Durada = rs2.getInt("Durada");
            int Data = rs2.getInt("Data");
            Date Data_fisica = rs2.getDate("Data_fisica");
            Date h_fi = rs2.getDate("h_fi");
            Date h_inici = rs2.getDate("h_inici");
            reserves = rs2.getInt("Reserves");
            int ID_s = rs2.getInt("ID_s");
            int ID = rs2.getInt("ID");
            int aforament = rs2.getInt("aforament");
            String descripcio2 = rs2.getString("descripcio");
            int restants = aforament-reserves;
            int Porcentaje = (int)((restants * aforament) / aforament);
            System.out.println("--------------------");
            System.out.println("ACTIVITAT ID: " + ID_a + " \nDescripcio: " + descripcio + " \nDurada: " + Durada + " \nData: " + Data + " \nData Física: " + Data_fisica + " \nH_fi: " + h_fi + " \nH_inici:" + h_inici + " \nReserves: " + reserves+ " \nID_s:" + ID_s + " \nAmb un: " + Porcentaje +"% de les Reserves Restants" );
            }
    }
